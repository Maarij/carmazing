...
public class SalesOrderCommandService {
    ...
    public SalesOrder addNewSalesOrder(AddSalesOrderInput addSalesOrderInput) {
        var existingCustomer = customerRepository.findById(UUID.fromString(addSalesOrderInput.getCustomerUuid()));

        if (existingCustomer.isEmpty()) {
            throw new DgsEntityNotFoundException(String.format("Customer UUID %s not found", addSalesOrderInput.getCustomerUuid()));
        }
